...
//		}
	...
	private String getDlnaOrgPnFlags(RendererConfiguration mediaRenderer, int localizationValue) {
		...
		if (mediaRenderer.isDLNAOrgPNUsed() || mediaRenderer.isAccurateDLNAOrgPN()) {
			if (mediaRenderer.isPS3()) {
				...
				} else if ((mime.length() > 9 && mime.substring(0, 9).equals(AUDIO_LPCM_TYPEMIME)) || mime.equals(AUDIO_WAV_TYPEMIME)) {
			...
			if (refCount == 0) {
				...
				Runnable r = new Runnable() {
					...
					public void run() {
						...
						notifyStartStopEvent(Event.Start);
					}
				};
	...
	private void notifyStartStopEvent(Event eventType) {
		NotificationCenter.getInstance(StartStopEvent.class).post(new StartStopEvent(this, eventType));		
	}
					...
					Runnable r = new Runnable() {
						...
						public void run() {
							if (refCount == 1) {
								...
								internalStop();
								notifyStartStopEvent(Event.Stop);
							}
						}
					};
