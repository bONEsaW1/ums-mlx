			...
			if (managedFolder.isWatchEnabled()) {
				try {
					...
					if (logger.isDebugEnabled())
						logger.debug(String.format("Started watching directory='%s'. Watch subfolders=%s", directoryToWatch, watchSubFolders));
				} catch (Throwable t) {
	...
	private void stopWatch(List<String> managedFolderPaths) {
		...
		for (String managedFolderPath : managedFolderPaths) {
			...
			try {
				...
				if (logger.isDebugEnabled())
					logger.debug(String.format("Stopped watching directory='%s'", managedFolderPath));
			} catch (Throwable t) {
