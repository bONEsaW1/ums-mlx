...
public class ETable extends JTable {
	...
	public ETable() {
		...
		// Worse, it's not easy (or possible?) for us to take over grid painting ourselves for those LAFs (Metal, for
		// example) that do paint grids.
	...
	public Component prepareRenderer(TableCellRenderer renderer, int row, int column) {
		...
		if (c instanceof JComponent) {
			...
			// Sun 5043225 says this is the intended behavior, though presumably not when it's being used as a table
			// cell renderer.
			...
			if (getCellSelectionEnabled() == false && isEditing() == false) {
				if (Platform.isMac()) {
					...
					// It does however draw a horizontal line under the whole row, and a vertical line separating each
					// column.
	...
	public Point getToolTipLocation(MouseEvent e) {
		// After a tool tip has been displayed for a cell that has a tool tip, cells without tool tips will show an
		// empty tool tip until the tool tip mode times out (or the table has a global default tool tip).
		// (ToolTipManager.checkForTipChange considers a non-null result from getToolTipText *or* a non-null result from
		// getToolTipLocation as implying that the tool tip should be displayed. This seems like a bug, but that's the
		// way it is.)
