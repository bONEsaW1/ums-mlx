...
public class AutoFolderCreator {
	...
	public static void addInitialFolderStructure(IMediaLibraryStorage storage) {
		if (log.isDebugEnabled())
			log.debug("Start creating the initial tree folder structure");
		...
		if (log.isInfoEnabled())
			log.info("The initial tree folder structure has been created");
	}
	...
	private static DOMediaLibraryFolder createAllFolder(DOMediaLibraryFolder parent, int positionInParent, FileDisplayProperties childProps, DOTemplate movieFileTemplate) {
		...
		FileDisplayProperties childMovieProps = new FileDisplayProperties("%name (%year) - %rating_percent/100 [%rating_voters]", true, FileDisplayType.FOLDER, false, ConditionType.FILE_DATEINSERTEDDB, false, movieFileTemplate, SortOption.FileProperty, false, false);
	...
	public static void addFileSystemFolders(IMediaLibraryStorage storage, DOMediaLibraryFolder parent, int insertPosition, File baseFolder) {
		if (log.isDebugEnabled())
			log.debug("Insert file system folders for folder '" + baseFolder.getAbsolutePath() + "' into parent " + parent.getName() + " (id=" + parent.getId() + ")");
		...
		if (baseFolder.exists() && baseFolder.isDirectory() && (!baseFolder.isHidden() || baseFolder.getAbsolutePath().endsWith(":\\"))) {
			...
			if (log.isInfoEnabled())
				log.info("File system folders inserted for folder '" + baseFolder.getAbsolutePath() + "' into parent " + parent.getName() + " (id=" + parent.getId() + ")");
	...
	public static void addTypeFolder(IMediaLibraryStorage storage, DOMediaLibraryFolder parent, AutoFolderProperty prop, boolean isAscending, int insertPos, int minOccurences) {
		if (log.isDebugEnabled())
			log.debug("Insert folder of type " + prop.toString() + ", isAscending= " + isAscending + " into parent " + parent.getName() + " (id=" + parent.getId() + ")");
		...
		for (String conStr : items) {
			...
			new DOMediaLibraryFolder(-1, newRootFolder, displayName, "c1", cons, true, true, newRootFolder.getFileType(), insertPosition++, displayProps, true, true);
		}
		storage.insertFolder(newRootFolder);
		if (log.isInfoEnabled())
			log.info("Folder of type " + prop.toString() + ", isAscending= " + isAscending + " inserted into parent " + parent.getName() + " (id=" + parent.getId() + ")");
	}
	...
	public static void addTagFolder(IMediaLibraryStorage storage, DOMediaLibraryFolder parent, String tagName, boolean isAscending, int insertPos, int minOccurences) {
		if (log.isDebugEnabled())
			log.debug("Insert folder for tag=" + tagName + ", isAscending= " + isAscending + " into parent " + parent.getName() + " (id=" + parent.getId() + ")");
		...
		// Create the base node
		FileDisplayProperties displayProps = new FileDisplayProperties(false, true);
		DOMediaLibraryFolder newRootFolder = new DOMediaLibraryFolder(-1, parent, tagName, "", new ArrayList<DOCondition>(), false, true, parent.getFileType(), insertPos, displayProps, true, true);
		...
		for (String tagValue : tagValues) {
			...
			new DOMediaLibraryFolder(-1, newRootFolder, tagValue, "c1", cons, true, true, newRootFolder.getFileType(), insertPosition++, displayProps, true, true);
		}
		storage.insertFolder(newRootFolder);
		if (log.isInfoEnabled())
			log.info("Folder for tag=" + tagName + ", isAscending= " + isAscending + " inserted into parent " + parent.getName() + " (id=" + parent.getId() + ")");
	}
}
